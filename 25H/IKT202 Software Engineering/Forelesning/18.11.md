...
input space -> system -> output partitions.
possible inputs ------->   possible outputs.

Top down
- gui down, uses stubs for lowe level
buottom up
- drivers
ad hoc
- new compoenents are added in integration test when finished
- stubs and driver
bacbone integration
- skeleton backbone is integrated with minimal functionality, grudually grows.
- verifies a given architecture, but may require a labor intensive bacbone
the big bbang - wait until everything is done

horizontal
vertical testing
	test one and one usecase.
	problem, if they(regression tests)