...
A centralized or global routing algorithm requires each node or router to maintain a full model of the topology of the network, including cost and calculating the shortest path often using Dijkstra's algorithm. A centralized routing algorithm is often easier to implement than a distributed routing algorithm. One example of a centralized routing algorithm is 
